 ...
 * Displays the current work session status and reactively computes duration metrics.
 */
...
onMounted(() => {
    // Updates the internal clock every second
    timer = setInterval(() => {
        now.value = new Date();
    }, 1000);
});
...
/**
 * Calculates effective working time (ignoring and subtracting pause durations).
 * Automatically freezes the timer if the session is currently paused.
 * @returns {string} Formatted HH:MM:SS string.
 */
const workingTimeFormatted = computed(() => {
    if (!props.session) return '00:00:00';
    
    // Defensive fallback to guarantee correct date parsing
    ...
    // Subtract time accumulated from all pauses
    if (props.session.pauses && props.session.pauses.length > 0) {
        props.session.pauses.forEach(pause => {
            ...
            // If the pause is ongoing, subtract until the current second
...
/**
 * Calculates elapsed time of the ongoing pause (if the user is currently paused).
 * @returns {string|null} Formatted MM:SS or HH:MM string.
 */
const currentPauseTimeFormatted = computed(() => {
    if (props.session?.status !== 'paused') return null;
    
    // Finds the pause that does not have an end date defined
    ...
    // Dynamic formatting if the pause exceeds one hour
    if (hours > 0) {
...
<template>
    <div class="flex items-center gap-2 px-3 py-1.5 rounded-full border shadow-sm text-sm font-medium transition-colors duration-300"
        ...
        <span class="uppercase tracking-wider text-[11px] font-bold">
            {{ session.status === 'active' ? $t('work_sessions.badge.active') : $t('work_sessions.badge.paused') }}
        </span>

        <span class="opacity-30">|</span>

        <span class="font-mono tabular-nums tracking-tight flex items-center gap-1" :title="$t('work_sessions.badge.worked_time')">
        ...
        <template v-if="session.status === 'paused'">
            <span class="opacity-30">|</span>
            <span class="text-xs opacity-90 font-mono tabular-nums flex items-center gap-1" :title="$t('work_sessions.badge.break_duration')">
